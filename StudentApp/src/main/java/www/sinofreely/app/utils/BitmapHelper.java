...
public class BitmapHelper {
	public static String TAG = "BitmapHelper";
	/**
	...
	public final static String compressBitmap(Context context, String srcPath, int rqsW, int rqsH, boolean isDelSrc,
		...
		String desPath = cachedir + File.separator + srcFile.getName();
		int degree = getDegress(srcPath);
		try {
			if (degree != 0)
				bitmap = rotateBitmap(bitmap, degree);
			Bitmap bmp = compressBitmap(bitmap, 1024 * 30);// 压缩到30K
	...
	public final static Bitmap compressBitmap(Bitmap bitmap, int reqsW, int reqsH, boolean isAdjust) {
		...
		if (bitmap.getWidth() > reqsW || bitmap.getHeight() > reqsH) {
			float scaleX = new BigDecimal(reqsW).divide(new BigDecimal(bitmap.getWidth()), 4, RoundingMode.DOWN)
					.floatValue();
	...
	public final static Bitmap compressBitmap(Bitmap bitmap, long maxBytes) {
		try {
			...
			int options = 90;
			while (baos.toByteArray().length > maxBytes && options >= 10) {
				System.out.println(options + "这是options");
				baos.reset();
